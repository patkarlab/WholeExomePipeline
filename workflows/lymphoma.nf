...
workflow LYMPHOMA {
	...
	main:
		...
		// VEP_PINDEL(PINDEL_FLT3.out, genome_loc, index_files)
		// FORMAT_PINDEL(PINDEL_FLT3.out.join(VEP_PINDEL.out))
		// SOMATICSEQ_LYMPHOMA(ABRA_SORT.out.final_bam.join(LOFREQ.out.join(VARSCAN.out.join(PLATYPUS.out.join(STRELKA.out.join(HAPLOTYPECALLER.out.join(FREEBAYES.out.join(DEEPSOMATIC.out))))))), genome_loc, index_files, bedfile, known_SNPs, known_SNPs_index)		
		...
		// CNVKIT(ABRA_SORT.out.final_bam, cnvkit_reference)
		// PLOT_CNVKIT(CNVKIT.out.cnvkit_files, gene_scatter_list)
		// CAVA(SOMATICSEQ_LYMPHOMA_CONCAT.out, cava_config, genome_loc, index_files, bedfile, bedfile_zipped, known_SNPs, known_SNPs_index)
