...
process COVERAGE_MOSDEPTH {
   tag "${Sample}"
	...
	script:
	"""
	${params.mosdepth_script} ${finalBam} ${Sample}_cov ${params.bedfile}
...
process COVERAGE_BEDTOOLS {
	...
	${params.bedtools} coverage -counts -a ${params.bedfile} -b ${Sample}_temp.bed > "${Sample}_cov.regions_bedtools.bed"
	"""
}
...
process COVERAGE_COVERVIEW {
	...
	script:
	"""
	${params.coverview_path}/coverview -i ${finalBam} -b ${params.bedfile} -c ${params.coverview_path}/config/config.txt -o ${Sample}.coverview
